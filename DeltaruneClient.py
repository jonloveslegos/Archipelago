                ...
                for file_name in os.listdir(tempInstall):
                    if file_name != "steam_api.dll":
                        shutil.copy(os.path.join(tempInstall, file_name),
                               Utils.user_path("Deltarune", file_name))
                self.ctx.patch_game()
...
class DeltaruneContext(CommonContext):
    ...
    def patch_game(self):
        with open(Utils.user_path("Deltarune", "data.win"), "rb") as f:
            patchedFile = bsdiff4.patch(f.read(), deltarune.data_path("patch.bsdiff"))
    ...
    def clear_deltarune_files(self):
        ...
        for root, dirs, files in os.walk(path):
            for file in files:
                ...
                elif file.endswith((".item", ".victory", ".playerspot", ".mine", ".flag", ".hint")):
                    os.remove(os.path.join(root, file))
    ...
    def run_gui(self):
        ...
        class DRManager(GameManager):
            logging_pairs = [
                ("Client", "Archipelago")
            ]
            base_title = "Archipelago Deltarune Client"

        self.ui = DRManager(self)
