...
def get_payload(ctx: KHDaysContext):
    ...
    print([''.join([i+" " for i in str(locations_by_id[item]).split(" ")[:-1]])[:-1] for item in ctx.checked_locations if (''.join([i+" " for i in str(locations_by_id[item]).split(" ")[:-1]])[:-1]+" "+str(int("".join(filter(str.isdigit, str(locations_by_id[item]))))-1)) in ctx.checked_locations or int("".join(filter(str.isdigit, str(locations_by_id[item])))) == 1])
    return json.dumps(
        {
            ...
            "checked_locs": [''.join([i+" " for i in str(locations_by_id[item]).split(" ")[:-1]])[:-1] for item in ctx.checked_locations if (''.join([i+" " for i in str(locations_by_id[item]).split(" ")[:-1]])[:-1]+" "+str(int("".join(filter(str.isdigit, str(locations_by_id[item]))))-1)) in ctx.checked_locations or int("".join(filter(str.isdigit, str(locations_by_id[item])))) == 1],
