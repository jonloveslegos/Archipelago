...
def _fnaf_world_can_access(state: CollectionState, player: int, name: str):
    if name == "Choppy's Woods":
        ...
        return _fnaf_world_can_access(state, player, "Choppy's Woods") and state.has("Dusting Fields Access", player)
        ...
        return _fnaf_world_can_access(state, player, "Lilygear Lake")
    else:
        return False
...
def set_rules(world: MultiWorld, player: int):
    ...
    set_rule(world.get_location("Withered Bonnie", player), lambda state: (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    ...
    set_rule(world.get_location("Fazbear Hills: Endo Shop 2", player), lambda state: world.hard_logic[player] or _fnaf_world_can_access(state, player, "Choppy's Woods") or _fnaf_world_can_access(state, player, "Lilygear Lake"))
    ...
    set_rule(world.get_location("Fazbear Hills: Lolbit Shop 2", player), lambda state: world.hard_logic[player] or _fnaf_world_can_access(state, player, "Choppy's Woods") or _fnaf_world_can_access(state, player, "Lilygear Lake"))
    ...
    set_rule(world.get_location("Choppy's Woods: Lolbit Shop 2", player), lambda state: world.hard_logic[player] or _fnaf_world_can_access(state, player, "Choppy's Woods") or _fnaf_world_can_access(state, player, "Lilygear Lake"))
    ...
    set_rule(world.get_location("Dusting Fields: Lolbit Shop 3", player), lambda state: _fnaf_world_can_access(state, player, "Dusting Fields") and (world.hard_logic[player] or _fnaf_world_can_access(state, player, "Blacktomb Yard")))
    ...
    set_rule(world.get_location("Withered Foxy", player), lambda state: (_fnaf_world_can_access(state, player, "Lilygear Lake") or _fnaf_world_can_access(state, player, "Choppy's Woods")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Withered Chica", player), lambda state: (_fnaf_world_can_access(state, player, "Lilygear Lake") or _fnaf_world_can_access(state, player, "Choppy's Woods")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Withered Freddy", player), lambda state: (_fnaf_world_can_access(state, player, "Lilygear Lake") or _fnaf_world_can_access(state, player, "Choppy's Woods")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Golden Freddy", player), lambda state: ((world.hard_logic[player] and _fnaf_world_can_access(state, player, "Choppy's Woods")) or _fnaf_world_can_access(state, player, "Lilygear Lake") or _fnaf_world_can_access(state, player, "Dusting Fields")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Paperpals", player), lambda state: ((world.hard_logic[player] and _fnaf_world_can_access(state, player, "Choppy's Woods")) or _fnaf_world_can_access(state, player, "Lilygear Lake") or _fnaf_world_can_access(state, player, "Dusting Fields")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Nightmare Freddy", player), lambda state: ((world.hard_logic[player] and _fnaf_world_can_access(state, player, "Choppy's Woods")) or _fnaf_world_can_access(state, player, "Lilygear Lake") or _fnaf_world_can_access(state, player, "Dusting Fields")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Shadow Freddy", player),
             lambda state: (_fnaf_world_can_access(state, player, "Lilygear Lake") or _fnaf_world_can_access(state, player, "Dusting Fields")) and (
                         not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    ...
    set_rule(world.get_location("Phantom Marionette", player),
             lambda state: (_fnaf_world_can_access(state, player, "Lilygear Lake") or _fnaf_world_can_access(state, player, "Dusting Fields")) and (
                         not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Crying Child", player), lambda state: _fnaf_world_can_access(state, player, "Choppy's Woods") and (world.hard_logic[player] or _fnaf_world_can_access(state, player, "Pinwheel Funhouse")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Funtime Foxy", player), lambda state: _fnaf_world_can_access(state, player, "Choppy's Woods") and (world.hard_logic[player] or _fnaf_world_can_access(state, player, "Pinwheel Funhouse")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Nightmare Fredbear", player), lambda state: _fnaf_world_can_access(state, player, "Choppy's Woods") and (world.hard_logic[player] or _fnaf_world_can_access(state, player, "Pinwheel Funhouse")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    ...
    set_rule(world.get_location("Nightmare Bonnie", player), lambda state: ((world.hard_logic[player] and _fnaf_world_can_access(state, player, "Choppy's Woods")) or _fnaf_world_can_access(state, player, "Lilygear Lake")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Nightmare Chica", player), lambda state: ((world.hard_logic[player] and _fnaf_world_can_access(state, player, "Choppy's Woods")) or _fnaf_world_can_access(state, player, "Lilygear Lake")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Nightmare Foxy", player), lambda state: ((world.hard_logic[player] and _fnaf_world_can_access(state, player, "Choppy's Woods")) or _fnaf_world_can_access(state, player, "Lilygear Lake")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Endo 01", player), lambda state: ((world.hard_logic[player] and _fnaf_world_can_access(state, player, "Choppy's Woods")) or _fnaf_world_can_access(state, player, "Lilygear Lake")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Endo 02", player), lambda state: ((world.hard_logic[player] and _fnaf_world_can_access(state, player, "Choppy's Woods")) or _fnaf_world_can_access(state, player, "Lilygear Lake")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Plushtrap", player), lambda state: ((world.hard_logic[player] and _fnaf_world_can_access(state, player, "Choppy's Woods")) or _fnaf_world_can_access(state, player, "Lilygear Lake")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Endoplush", player), lambda state: ((world.hard_logic[player] and _fnaf_world_can_access(state, player, "Choppy's Woods")) or _fnaf_world_can_access(state, player, "Lilygear Lake")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Springtrap", player), lambda state: ((world.hard_logic[player] and _fnaf_world_can_access(state, player, "Choppy's Woods")) or _fnaf_world_can_access(state, player, "Lilygear Lake")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("RWQFSFASXC", player), lambda state: ((world.hard_logic[player] and _fnaf_world_can_access(state, player, "Choppy's Woods")) or _fnaf_world_can_access(state, player, "Lilygear Lake")) and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    ...
    set_rule(world.get_location("Nightmare", player), lambda state: _fnaf_world_can_access(state, player, "Pinwheel Funhouse") and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Fredbear", player), lambda state: _fnaf_world_can_access(state, player, "Pinwheel Funhouse") and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
    set_rule(world.get_location("Spring Bonnie", player), lambda state: _fnaf_world_can_access(state, player, "Pinwheel Funhouse") and (not world.require_find_char[player] or _fnaf_world_has_chip(state, player, "Find Characters")))
